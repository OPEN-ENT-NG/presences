-- Attention ces tests vont supprimer les tables alerts, alert_history, event, forgotten_notebook
do $$
    declare
        ...
        countAlert bigint = 0;
        date_event_1 timestamp without time zone;
        date_event_2 timestamp without time zone;
        dateTmp timestamp without time zone;
        date_forgotten_notebook timestamp without time zone;
    ...
    begin
        ...
        TRUNCATE TABLE presences.event;
        TRUNCATE TABLE presences.forgotten_notebook;
        ...
        date_forgotten_notebook = now();
        INSERT INTO presences.forgotten_notebook(id, date, student_id, structure_id) VALUES (9699, date_forgotten_notebook, studentId1, structureId);
        SELECT a.date FROM presences.alerts as a LIMIT 1 INTO dateTmp;
        assert dateTmp::date = date_forgotten_notebook::date, 'create alert with the date of the associated forgotten_notebook';
        UPDATE presences.forgotten_notebook SET date = date_forgotten_notebook - INTERVAL '1 day' WHERE id = 9699;
        SELECT a.date FROM presences.alerts as a LIMIT 1 INTO dateTmp;
        assert dateTmp::date = date_forgotten_notebook::date - INTERVAL '1 day', 'update alert when forgotten_notebook date is update';
        INSERT INTO presences.forgotten_notebook(id, date, student_id, structure_id) VALUES (9698, date_forgotten_notebook, studentId2, structureId);
        INSERT INTO presences.forgotten_notebook(id, date, student_id, structure_id) VALUES (9697, date_forgotten_notebook - INTERVAL '2 day', studentId1, structureId);
        ...
        -- Test incidents
        date_incident = now();
        INSERT INTO incidents.incident(id, owner, structure_id, date, seriousness_id) VALUES (9599, '', structureId, date_incident, 9899);
        INSERT INTO incidents.incident(id, owner, structure_id, date, seriousness_id) VALUES (9598, '', structureId, date_incident, 9898);
        INSERT INTO incidents.protagonist(user_id, incident_id, type_id) VALUES (studentId1, 9599, 9799);
        SELECT a.date FROM presences.alerts as a WHERE event_id = 9599 LIMIT 1 INTO dateTmp;
        assert dateTmp = date_incident, 'create alert with the date of the associated incident';
        UPDATE incidents.incident SET date = date_incident - INTERVAL '1 day' WHERE id = 9599;
        SELECT a.date FROM presences.alerts as a LIMIT 1 INTO dateTmp;
        assert dateTmp = date_incident - INTERVAL '1 day', 'update alert when incident date is update';
        ...
        --Test event
        date_event_1 = now() - INTERVAL '10 day';
        date_event_2 = now() - INTERVAL '11 day';
        INSERT INTO presences.register(id, personnel_id, course_id, state_id, owner, structure_id, start_date) VALUES (9399, '', '', 3, '', structureId, date_event_1);
        INSERT INTO presences.register(id, personnel_id, course_id, state_id, owner, structure_id, start_date) VALUES (9398, '', '', 3, '', structureId, date_event_2);
        ...
        SELECT a.date FROM presences.alerts as a LIMIT 1 INTO dateTmp;
        assert dateTmp = date_event_2, 'create alert with the date of the associated event';
        ...
        INSERT INTO presences.event(id, start_date, end_date, student_id, register_id, type_id, reason_id, owner, counsellor_regularisation)
        VALUES (9498, now()::date + '09:00:00'::time, now()::date + '09:55:00'::time, studentId2, 9399, 1, NULL, '', false);
        SELECT count(*) FROM presences.alerts INTO countAlert;
        ...
        SELECT a.date FROM presences.alerts as a LIMIT 1 INTO dateTmp;
        assert dateTmp = date_event_1, 'create alert with the date of the associated event new event when old event no longer count in alerts';
        ...
        --Test history
        INSERT INTO presences.alerts(id, student_id, structure_id, type, created, event_id, date) VALUES (1, studentId1, structureId, 'ABSENCE', now(), 1, now() - INTERVAL '10 day');
        ...
        INSERT INTO presences.alerts(id, student_id, structure_id, type, created, event_id, date) VALUES (1, studentId1, structureId, 'ABSENCE', now(), 1, now() - INTERVAL '10 day');
        INSERT INTO presences.alerts(id, student_id, structure_id, type, created, event_id, date) VALUES (2, studentId1, structureId, 'ABSENCE', now(), 2, now() - INTERVAL '10 day');
        INSERT INTO presences.alerts(id, student_id, structure_id, type, created, event_id, date) VALUES (3, studentId1, structureId, 'ABSENCE', now(), 3, now() - INTERVAL '10 day');
        INSERT INTO presences.alerts(id, student_id, structure_id, type, created, event_id, date) VALUES (4, studentId1, structureId, 'ABSENCE', now(), 4, now() - INTERVAL '10 day');
        DELETE FROM presences.alerts WHERE id = 1;
        SELECT count(*) FROM presences.alert_history INTO countAlert;
        ...
        INSERT INTO presences.alerts(id, student_id, structure_id, type, created, event_id, date) VALUES (1, studentId1, structureId, 'ABSENCE', now(), 1, now() - INTERVAL '10 day');
        DELETE FROM presences.alerts WHERE 1 = 1; -- Can't use truncate because truncate do not active trigger.
        SELECT count(*) FROM presences.alert_history INTO countAlert;
