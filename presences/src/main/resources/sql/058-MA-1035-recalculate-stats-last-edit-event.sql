ALTER TABLE presences_statistics."user" add modified timestamp without time zone NOT NULL default now();
...
BEGIN
    ...
    IF stat_exists THEN
        ...
        ON CONFLICT (id, structure)
            DO UPDATE SET modified = OLD.start_date;
    END IF;
...
BEGIN
    ...
    IF stat_exists THEN
        ...
        ON CONFLICT (id, structure)
            DO UPDATE SET modified = NEW.start_date;
    END IF;
