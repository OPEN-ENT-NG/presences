    ...
    function ($scope, $route, $location, $timeout, GroupService: GroupService,
       ...
       ---------------------------- */
        ...
        vm.getAbsenceReasons = function (reasonArray: Reason[]): Reason[] {
            if (!reasonArray)
                return [];
            return reasonArray.filter(reason => !reason.reason_type_id || reason.reason_type_id == REASON_TYPE_ID.ABSENCE);
        }
        ...
        vm.getLatenessReasons = function (reasonArray: Reason[]): Reason[] {
            if (!reasonArray)
                return [];
            return reasonArray.filter(reason => !reason.reason_type_id || reason.reason_type_id == REASON_TYPE_ID.LATENESS);
        }
