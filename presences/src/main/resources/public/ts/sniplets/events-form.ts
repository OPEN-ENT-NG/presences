...
interface ViewModel {
    ...
    deleteEvent(eventType: TEventType, canReload: boolean): Promise<void>;
...
const vm: ViewModel = {
    ...
    editEvent(eventType: TEventType): void {
        const formEventType : TEventType = vm.form.eventType;
        switch (eventType) {
            case EVENT_TYPE.ABSENCE:
                if (formEventType !== EVENT_TYPE.ABSENCE) {
                ...
                break;
            case EVENT_TYPE.LATENESS:
                if (formEventType !== EVENT_TYPE.LATENESS) {
                    if (formEventType === EVENT_TYPE.ABSENCE) {
                        vm.event = new Absence(null, null, null, null);
                    }
                    vm.deleteEvent(formEventType, false)
    ...
    async deleteEvent(eventType: TEventType, canReload: boolean): Promise<void> {
        switch (eventType) {
            case EVENT_TYPE.ABSENCE:
                return vm.deleteAbsence(canReload);
            case EVENT_TYPE.LATENESS:
                return vm.deleteLateness(canReload);
        }
    },
