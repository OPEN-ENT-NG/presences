...
public class PresencesExportWorker extends ExportWorker {
   ...
   protected String getFolderName() {
    ...
    private Future<ExportFile> getEvents(String type, JsonObject params) {
        ...
        List<String> eventType = params.getJsonArray(Field.EVENTTYPE).getList();
        List<String> reasonIds = params.getJsonArray(Field.REASONIDS).getList();
        Boolean noReason = params.getBoolean(Field.NOREASON);
        Boolean noReasonLateness = params.getBoolean(Field.NOREASONLATENESS);
        List<String> userIds = params.getJsonArray(Field.USERIDS).getList();
        JsonArray userIdFromClasses = params.getJsonArray(Field.USERIDFROMCLASSES);
        List<String> classes = params.getJsonArray(Field.CLASSES).getList();
        List<String> restrictedClasses = params.getJsonArray(Field.RESTRICTEDCLASSES).getList();
