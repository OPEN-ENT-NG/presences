...
public class DefaultReasonService implements ReasonService {
                ...
                .onSuccess(res -> {
                    ...
                    final Map<Long, JsonArray> mapReasonIdAlertRule = reasonAlertRule.stream()
                            ...
                            .stream().collect(Collectors.toMap(Map.Entry::getKey, stringListEntry -> {
                                List<String> listAlertRule = stringListEntry.getValue().stream()
                                        .map(jsonObject -> jsonObject.getString(Field.RULE_TYPE)).collect(Collectors.toList());
                                return new JsonArray(listAlertRule);
                            }));
                    for (int i = 0; i < reasons.size(); i++) {
                        JsonObject jsonObjectReason = reasons.getJsonObject(i);
                        JsonArray reasonAlertRules = mapReasonIdAlertRule.getOrDefault(jsonObjectReason.getLong(Field.ID), new JsonArray());
                        jsonObjectReason.put(Field.REASON_ALERT_RULES, reasonAlertRules);
    ...
    private Future<JsonArray> fetchReasonAlertRule(Integer reasonTypeId) {
        ...
        String query = "SELECT r.rule_type, reason_alert.reason_id FROM " + Presences.dbSchema + ".reason_alert" +
                " INNER JOIN " + Presences.dbSchema + ".reason_alert_exclude_rules_type as r" +
                " ON r.id = reason_alert.reason_alert_exclude_rules_type_id" +
        ...
        createReasonFuture
                .onSuccess(res -> {
                    ...
                    Arrays.stream(ReasonAlertExcludeRulesType.values())
                            ...
                            .map(reasonAlertExcludeRulesType -> {
                                return createOrUpdateReasonAlert(reasonId, structureId, reasonAlertExcludeRulesType, true);
                            })
    ...
    private Future<JsonObject> createOrUpdateReasonAlert(int reasonId, String structureId,
                                                         ReasonAlertExcludeRulesType reasonAlertExcludeRulesType, boolean exclude) {
        Promise<JsonObject> promise = Promise.promise();
        String query;
        JsonArray params;
        if (exclude){
            query = "INSERT INTO " + Presences.dbSchema + ".reason_alert(structure_id, reason_id, reason_alert_exclude_rules_type_id)" +
                    " VALUES (?, ?, " + reasonAlertExcludeRulesType.getValue() + ") ON CONFLICT ON CONSTRAINT uniq_reason_alert" +
                    " DO UPDATE SET deleted_at = null WHERE reason_alert.structure_id = ? AND reason_alert.reason_id = ?" +
                    " AND reason_alert.reason_alert_exclude_rules_type_id = " + reasonAlertExcludeRulesType.getValue();
            ...
            query = "UPDATE " + Presences.dbSchema + ".reason_alert SET deleted_at = now() WHERE structure_id = ?" +
                    " AND reason_id = ? AND reason_alert_exclude_rules_type_id = " + reasonAlertExcludeRulesType.getValue();
            params = new JsonArray()
                    .add(structureId)
                    .add(reasonId);
        }
