...
export function createDevProxyConfig({
  ...
  // NOTE: `headers` is applied via `server.headers` to EVERY dev response.
  // Set ONLY `set-cookie` here. Do NOT add `Cache-Control` — a max-age would
  // also cache `template/*.html` files entcore fetches at runtime, so HTML
  // template edits would not reflect across reloads until expiry. Vite's
  // default no-cache for those templates is what you want.
