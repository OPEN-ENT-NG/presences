...
public abstract class Indicator extends DBService {
    ...
    public void export(HttpServerRequest request, Filter filter, List<JsonObject> values, JsonObject count,
                       JsonObject slots, JsonObject rate) throws ClassNotFoundException, IllegalAccessException,
            InvocationTargetException, InstantiationException {
        String className = String.format(CSV_EXPORT_FORMATTER, name);
        ClassLoader loader = Indicator.class.getClassLoader();
        CSVExport export = (count != null) ?
                (CSVExport) Class.forName(className, true, loader).getConstructors()[0].newInstance(filter, values, count, slots, rate):
