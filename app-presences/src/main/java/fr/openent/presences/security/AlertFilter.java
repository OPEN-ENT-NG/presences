...
public class AlertFilter implements ResourcesProvider {
    @Override
    public void authorize(HttpServerRequest httpServerRequest, Binding binding, UserInfos userInfos, Handler<Boolean> handler) {
        ...
        handler.handle(userInfos.getStructures().contains(structureId) && WorkflowHelper.hasRight(userInfos, WorkflowActions.ALERT.toString()));
    }
}
