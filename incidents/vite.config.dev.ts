...
export default ({ mode }: { mode: string }) => {
  const { headers, proxy } = createDevProxyConfig({
    ...
    routes: [
      ...
      "^/(?=presences|massmailing|viescolaire|statistics-presences)",
      "^/incidents/(?!public/)",
    ],
  });
  ...
  return defineConfig({
    ...
    resolve: {
      ...
      alias: {
        ...
        "@incidents": path.resolve(__dirname, "./src/main/resources/public/ts"),
    ...
    plugins: [
      ...
      appPrefixRewritePlugin({
        appName: APP_NAME,
        // ng-app.js loads behaviours.js for each cross-imported module.
        // Stub all of them: loading a legacy webpack bundle would embed its own
        // AngularJS copy and trigger "AngularJS loaded more than once".
